...
export function runWithAssessmentRuntimeHidden(workspacePath, operation) {
  ...
  const holdingDirectory = fs.mkdtempSync(
    path.join(path.dirname(path.resolve(workspacePath)), ".appcat-runtime-"),
  );
  const hiddenRuntimePath = path.join(holdingDirectory, ".runtime");
  fs.renameSync(runtimePath, hiddenRuntimePath);
  ...
  let restoreError;
  try {
    fs.renameSync(hiddenRuntimePath, runtimePath);
  } catch (error) {
    restoreError = error;
  }
  if (!restoreError) {
    fs.rmSync(holdingDirectory, { recursive: true, force: true });
  }

  if (operationError && restoreError) {
    throw new AggregateError(
      [operationError, restoreError],
      `AppCAT failed and the assessment runtime could not be restored from ${hiddenRuntimePath}`,
    );
  }
  if (restoreError) {
    throw new Error(
      `The assessment runtime could not be restored from ${hiddenRuntimePath}: ${restoreError.message}`,
      { cause: restoreError },
    );
  }
  if (operationError) {
...
export function runAppcat({
  ...
  if (normalizedLanguage === "java") {
    ...
    try {
      ...
      const commandResult = runWithAssessmentRuntimeHidden(
        workspacePath,
        () => runCommand(check.executablePath, args, { cwd: workspacePath }),
      );
