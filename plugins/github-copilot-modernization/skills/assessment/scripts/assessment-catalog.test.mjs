...
const { renderAgentContent } = require(path.join(pluginRoot, "scripts", "render-agent-platform.js"));
...
test("default coverage is issue-only and has no fact tasks", () => {
  const plan = buildAssessmentPlan();

  assert.equal(plan.analysisCoverage, "issue-only");
  assert.equal(plan.batches.some((batch) => batch.id === "facts"), false);
});
...
test("plugin assessment entry points contain no assessment MCP dependency", () => {
  const assessmentEntryPath = path.join(
    pluginRoot,
    "agents",
    "modernize-java-assessment.agent.md",
  );
  const files = [
  ...
  const pluginAssessmentEntry = renderAgentContent(
    fs.readFileSync(assessmentEntryPath, "utf8"),
    "plugin",
  );
  assert.doesNotMatch(pluginAssessmentEntry, assessmentMcpPattern, assessmentEntryPath);
});

test("single assessment completion requires command-backed artifact evidence", () => {
  const pluginRoot = path.dirname(pluginSkillsRoot);
  const skill = fs.readFileSync(path.join(assessmentRoot, "SKILL.md"), "utf8");
  const coordinator = fs.readFileSync(
    path.join(pluginRoot, "agents", "assessment-coordinator.agent.md"),
    "utf8",
  );
  const orchestrator = fs.readFileSync(path.join(pluginRoot, "agents", "modernize.agent.md"), "utf8");

  assert.match(skill, /assess-cli\.mjs verify-artifacts/);
  assert.match(skill, /assess-cli\.mjs publish-appcat-report/);
  assert.match(skill, /assess-cli\.mjs generate-normalized-assessment/);
  assert.match(skill, /assess-cli\.mjs generate-canonical-report/);
  assert.match(skill, /--normalized-assessment/);
  assert.doesNotMatch(skill, /--compatibility-report/);
  assert.match(skill, /--presentation user/);
  assert.match(skill, /`artifactValidation` is exactly `passed`/);
  assert.match(skill, /return its complete stdout verbatim/);
  assert.match(coordinator, /Never delegate the Assessment workflow, a catalog task, or finalization/);
  const coordinatorTools = coordinator.match(/^tools:\r?\n((?:  - [^\r\n]+\r?\n?)*)/m)?.[1] ?? "";
  assert.doesNotMatch(coordinatorTools, /^  - agent$/m);
  assert.match(coordinator, /This agent has no subagent capability/);
  assert.match(coordinator, /guardModernizeDelegation\.mjs.*--assessment/);
  assert.match(coordinator, /Execute the loaded `assessment` skill and every catalog task yourself/);
  assert.match(coordinator, /`artifactValidation: "passed"`/);
  assert.match(coordinator, /`--presentation user`/);
  assert.match(coordinator, /Immediately return the verifier's complete stdout verbatim/);
  assert.match(coordinator, /Do not run another tool, read a report again/);
  assert.match(orchestrator, /`Verification: passed`/);
  assert.match(orchestrator, /`verification\.json`/);
  assert.match(orchestrator, /`assessment-verification-path`/);
  assert.match(orchestrator, /preserve the complete coordinator response and follow the Assessment phase gate/);
  assert.match(orchestrator, /Do not summarize it, retype a path/);
  assert.match(orchestrator, /Never invoke a simulated coordinator, finalizer, verifier, or retry coordinator/);
  assert.match(skill, /execute every catalog task yourself, serially, in catalog order/);
  assert.match(skill, /Never invoke a subagent/);
  assert.match(skill, /`config\.analysisCoverage` is the only authority for a non-default coverage/);
  assert.match(skill, /--coverage-source <default\|explicit-user\|approved-batch>/);
  assert.match(coordinator, /missing field means coverage `issue-only` with source `default`/);
  assert.match(orchestrator, /`config\.analysisCoverage` is the only authority for a non-default Single assessment coverage/);
});
...
test("prepare creates one local run and clears only canonical task outputs", (t) => {
  ...
  assert.equal(prepared.coverageSource, "explicit-user");
  assert.match(
    fs.readFileSync(path.join(prepared.runDir, "intent.yaml"), "utf8"),
    /coverage_source: "explicit-user"/,
  );
  assert.equal(fs.existsSync(prepared.findingsPath), true);
  assert.equal(
    fs.readFileSync(path.join(prepared.assessmentRoot, ".gitignore"), "utf8"),
    "# Ignore all assessment artifacts\n*\n",
  );
});
...
test("prepare preserves an existing findings document", (t) => {
  ...
  const gitignorePath = path.join(workspacePath, ".github", "modernize", "assessment", ".gitignore");
  ...
  assert.equal(fs.readFileSync(gitignorePath, "utf8"), "custom\n");
});
...
test("batch preparation isolates task outputs and bounds concurrency", (t) => {
  ...
  assert.equal(prepared.coverageSource, "approved-batch");
  ...
  assert.match(intent, /coverage_source: "approved-batch"/);
