...
const SUPPORTED_COVERAGE_SOURCES = new Set(["default", "explicit-user", "approved-batch"]);
...
function writeRunMetadata({ runDir, runId, language, plan }) {
  ...
  const intent = [
    ...
    `coverage_source: ${yamlScalar(plan.coverageSource)}`,
...
function createAssessmentGitignore(assessmentRoot) {
  const gitignorePath = path.join(assessmentRoot, ".gitignore");
  let descriptor;
  try {
    descriptor = fs.openSync(gitignorePath, "wx", 0o600);
    fs.writeFileSync(descriptor, "# Ignore all assessment artifacts\n*\n", "utf8");
  } catch (error) {
    if (error?.code !== "EEXIST") throw error;
  } finally {
    if (descriptor !== undefined) fs.closeSync(descriptor);
...
export function prepareAssessmentRun({
  ...
  const resolvedScratchRoot = attemptScratchRoot
    ...
    : null;
  const resolvedCoverageSource = coverageSource
    ?? (resolvedScratchRoot ? "approved-batch" : "default");
  if (!SUPPORTED_COVERAGE_SOURCES.has(resolvedCoverageSource)) {
    throw new Error(`Unsupported coverage source: ${resolvedCoverageSource}`);
  }
  if (analysisCoverage === "full" && resolvedCoverageSource === "default") {
    throw new Error("Full coverage requires an explicit-user or approved-batch source");
  }
  ...
  plan.coverageSource = resolvedCoverageSource;

  fs.mkdirSync(assessmentRoot, { recursive: true });
  fs.mkdirSync(memoryDir, { recursive: true });
  if (resolvedScratchRoot) fs.mkdirSync(resolvedScratchRoot, { recursive: true });
  createAssessmentGitignore(assessmentRoot);
