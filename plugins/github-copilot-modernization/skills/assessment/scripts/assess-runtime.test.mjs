...
import {
  ...
  runWithAssessmentRuntimeHidden,
...
test("runWithAssessmentRuntimeHidden restores the runtime after AppCAT completes", () => {
  const root = temporaryDirectory();
  ...
  writeFile(runtimeFile, "runtime");

  const result = runWithAssessmentRuntimeHidden(workspacePath, () => {
    assert.equal(fs.existsSync(runtimeFile), false);
    return "completed";
  });

  assert.equal(result, "completed");
  assert.equal(fs.readFileSync(runtimeFile, "utf8"), "runtime");
  assert.deepEqual(
    fs.readdirSync(root).filter((name) => name.startsWith(".appcat-runtime-")),
    [],
  );
});

test("runWithAssessmentRuntimeHidden restores the runtime after AppCAT fails", () => {
  const root = temporaryDirectory();
  const workspacePath = path.join(root, "project");
  const runtimeFile = path.join(
    workspacePath,
  ...
  assert.throws(
    () => runWithAssessmentRuntimeHidden(workspacePath, () => {
      assert.equal(fs.existsSync(runtimeFile), false);
      throw new Error("AppCAT failed");
    }),
    /AppCAT failed/,
  );
...
test("Java AppCAT arguments do not turn release metadata defaults into analysis filters", () => {
  const args = buildJavaAppcatArgs({
    workspacePath: "C:\\src",
    outputPath: "C:\\out",
  });

  assert.deepEqual(args.slice(args.indexOf("--target"), args.indexOf("--overwrite")), [
    "--target",
