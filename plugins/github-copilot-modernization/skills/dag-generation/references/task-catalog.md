...
### smoke-test
- **desc**: Independent build and startup verification from reviewer perspective. Run build, start the application, verify it responds on expected port. Record results (build status, startup time, HTTP status) in artifact. This is NOT testing — it is a gate check that the deliverable is minimally viable.
- **scope**: global
