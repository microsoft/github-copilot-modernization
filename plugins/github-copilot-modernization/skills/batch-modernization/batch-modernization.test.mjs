...
test("Batch Assessment retains the complete deterministic control-plane surface", () => {
  const expected = [
    ...
    "schemas/normalized-assessment.v1.json",
...
function frontmatterTools(content) {
  ...
  const tools = frontmatter.match(/^tools:\r?\n((?:  - [^\r\n]+\r?\n?)*)/m)?.[1] ?? "";
  return [...tools.matchAll(/^  - ([^\r\n]+)$/gm)].map((match) => match[1]);
}
...
test("Batch Assessment routes default config through a top-level mode choice", () => {
  ...
  const modernize = renderAgentContent(fs.readFileSync(modernizePath, "utf8"), "plugin");
  ...
  assert.match(modernize, /`modernize` is the default conversational entry point/);
  assert.match(modernize, /Dedicated workflows may select `modernize-java-assessment`, `modernize-azure-java`, `modernize-azure-dotnet`, `modernize-java-upgrade`, `modernize-java-security`, `modernize-deployment`, `modernize-rearchitecture`, or `modernize-websphere-standalone` directly/);
  assert.match(modernize, /All other agents remain internal implementation details/);
  ...
  assert.match(modernize, /`status: found`.*invoke `#vscode\/askQuestions` in VS Code, or `#ask_user`/s);
  ...
  assert.match(modernize, /immediate next action after a valid Review is to invoke the top-level question tool/);
  ...
  assert.match(modernize, /If and only if the top-level host exposes neither `vscode\/askQuestions` nor `ask_user`/);
  ...
  assert.match(modernize, /current host does not expose a question tool inside a nested agent invocation/);
  ...
  const modernizeFrontmatter = modernize.match(/^---\r?\n([\s\S]*?)\r?\n---/)?.[1] ?? "";
  assert.deepEqual(frontmatterTools(modernize), []);
  assert.doesNotMatch(modernizeFrontmatter, /^agents:/m);
  ...
  assert.match(coordinator, /agents:\s*\n\s+- batch-assessment/);
  ...
  assert.equal(assessmentTools.includes("agent"), false);
  ...
  assert.match(review, /immediate next and only tool action is one foreground terminal command invoking the absolute skill-owned `prepare-review\.mjs`/);
  assert.match(review, /Never guess a plugin installation root or read `CLAUDE_PLUGIN_ROOT`, `COPILOT_PLUGIN_ROOT`, or `PLUGIN_ROOT`/);
  ...
  assert.match(review, /inspectedReposSha256: <64 lowercase hex characters>/);
  ...
  assert.match(assessment, /load the `assessment` skill in `batch-headless` mode/);
  assert.match(assessment, /optional `targetRuntime`, `targetComputeServices`, `enableContainerization`, `targetOS`, `minimumCveSeverity`, and `cveScanScope`/);
  assert.match(assessment, /Execute every task yourself, serially, in catalog order/);
  assert.match(assessment, /For full coverage, archive facts/);
  assert.match(coordinator, /Do not require Review text in the prompt/);
  assert.match(coordinator, /Treat `review\.json`, `REVIEW\.md`, and `inspected-repos\.json` as opaque inputs to `initialize-assessment`/);
  assert.match(coordinator, /The deterministic initializer is the sole authority/);
  assert.doesNotMatch(coordinator, /Read `review\.json` as the stable Review authority/);
  ...
  assert.match(coordinator, /Do not create `selection\.json`, copy decisions into the input/);
  assert.match(coordinator, /derives `batchId`, selected execution units, attention approvals, and decisions directly from `review\.json`/);
  ...
  assert.match(assessment, /authorized and required to run the request-bound Node commands that create or update generated Assessment runtime and report artifacts/);
  assert.match(assessment, /Never manually delete, rename, replace, or edit an existing `\.github` path/);
  ...
  assert.match(assessment, /"status": "completed",\s+"artifacts":/s);
  assert.match(assessment, /"needsInput": null/);
  assert.match(assessment, /boolean `retryable`/);
  assert.match(assessment, /resolve `\.\.\/\.\.\/batch-modernization\/scripts\/batch-attempt\.mjs`/);
  assert.match(assessment, /node <batchAttemptScriptPath> publish/);
  assert.doesNotMatch(assessment, /node <plugin-root>\/skills\/batch-modernization\/scripts\/batch-attempt\.mjs publish/);
});