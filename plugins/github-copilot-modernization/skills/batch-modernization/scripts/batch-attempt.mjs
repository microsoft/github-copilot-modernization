...
function validateInitializationInput(input) {
  if (!input || typeof input !== "object" || Array.isArray(input)) {
    throw new BatchStateError("Initialization input must be an object", "invalid_attempt_input");
  }
  const unknownFields = Object.keys(input).filter((name) => name !== "userRequest");
  if (unknownFields.length > 0) {
    throw new BatchStateError(
      `Initialization input contains unsupported fields: ${unknownFields.join(", ")}`,
      "invalid_attempt_input",
    );
  }
  if (typeof input.userRequest !== "string" || !input.userRequest.trim()) {
    throw new BatchStateError("Initialization input requires userRequest", "invalid_attempt_input");
  }
}

function plainFileSha256(filePath) {
  return crypto.createHash("sha256").update(fs.readFileSync(filePath)).digest("hex");
}

function requireApprovedDigest(value, label) {
  if (typeof value !== "string" || !/^[a-f0-9]{64}$/.test(value)) {
    throw new BatchStateError(`${label} must be a lowercase SHA-256 digest`, "invalid_review_digest");
  }
  return value;
}

function approvedReviewAuthority({
  batchRoot,
  resolvedConfig,
  reviewPath,
  reviewSha256,
  ...
  const absoluteBatchRoot = path.resolve(batchRoot ?? "");
  const absoluteReviewPath = path.resolve(reviewPath ?? "");
  if (absoluteReviewPath !== path.join(absoluteBatchRoot, "review.json")) {
    throw new BatchStateError("Approved Review path does not match the batch root", "invalid_review_path");
  }
  const expectedReviewSha256 = requireApprovedDigest(reviewSha256, "Review digest");
  if (plainFileSha256(absoluteReviewPath) !== expectedReviewSha256) {
    throw new BatchStateError("Approved Review digest does not match review.json", "review_digest_mismatch");
  }
  const review = jsonDocument(absoluteReviewPath, "approved Review");
  const absoluteReviewMarkdownPath = path.resolve(review.reviewMarkdownPath ?? "");
  if (absoluteReviewMarkdownPath !== path.join(absoluteBatchRoot, "REVIEW.md")) {
    throw new BatchStateError("Approved Review Markdown path does not match the batch root", "invalid_review_path");
  }
  const expectedReviewMarkdownSha256 = requireApprovedDigest(
    reviewMarkdownSha256,
    "Review Markdown digest",
  );
  if (plainFileSha256(absoluteReviewMarkdownPath) !== expectedReviewMarkdownSha256) {
    throw new BatchStateError(
      "Approved Review Markdown digest does not match REVIEW.md",
      "review_digest_mismatch",
    );
  }
  if (review.status !== "ready_for_approval"
      || path.resolve(review.batchRoot ?? "") !== absoluteBatchRoot
      || path.resolve(review.reviewPath ?? "") !== absoluteReviewPath
      || path.resolve(review.batchAttemptScriptPath ?? "") !== path.resolve(fileURLToPath(import.meta.url))) {
    throw new BatchStateError("Approved Review identity does not match initialization", "review_identity_mismatch");
  }
  if (typeof review.markdown !== "string"
      || fs.readFileSync(absoluteReviewMarkdownPath, "utf8") !== `${review.markdown}\n`) {
    throw new BatchStateError("Approved Review JSON and Markdown differ", "review_content_mismatch");
  }
  const inspectedReviewPath = path.resolve(review.inspectedReposPath ?? "");
  if (inspectedReviewPath !== path.join(absoluteBatchRoot, "scratch", "inspected-repos.json")) {
    throw new BatchStateError("Approved inspected repositories path is invalid", "invalid_review_path");
  }
  const approvedInspected = validatedDocument(
    inspectedReviewPath,
    resolvedSchema,
    resolvedSchemaPath,
    "approved inspected repositories",
  );
  const expectedInspectedSha256 = requireApprovedDigest(
    review.inspectedReposSha256,
    "Inspected repositories digest",
  );
  if (plainFileSha256(inspectedReviewPath) !== expectedInspectedSha256) {
    throw new BatchStateError(
      "Approved inspected repositories digest does not match inspected-repos.json",
      "review_digest_mismatch",
    );
  }
  if (review.configSha256 !== approvedInspected.configSha256
      || review.configSha256 !== resolvedConfig.configSha256) {
    throw new BatchStateError("Approved Review config digest does not match inspection", "review_config_mismatch");
  }
  validateAssessmentDecisions(review.decisions);
  const selectedIds = review.selectedExecutionUnitIds;
  const approvedNeedsAttention = review.approvedNeedsAttention;
  if (!Array.isArray(selectedIds)
      || selectedIds.length === 0
      || new Set(selectedIds).size !== selectedIds.length) {
    throw new BatchStateError("Approved Review requires unique execution-unit IDs", "invalid_review_selection");
  }
  if (!Array.isArray(approvedNeedsAttention)
      || new Set(approvedNeedsAttention).size !== approvedNeedsAttention.length
      || approvedNeedsAttention.some((executionUnitId) => !selectedIds.includes(executionUnitId))) {
    throw new BatchStateError("Approved attention IDs must be a subset of the Review selection", "invalid_review_selection");
  }
  return {
    review,
    approvedInspected,
    reviewPath: absoluteReviewPath,
    reviewSha256: expectedReviewSha256,
    reviewMarkdownPath: absoluteReviewMarkdownPath,
    reviewMarkdownSha256: expectedReviewMarkdownSha256,
  };
}
...
export function initializeAssessmentBatch({
  ...
  if (selection !== undefined) {
    throw new BatchStateError(
      "Independent selection is not accepted; initialization derives it from the approved Review",
  ...
  const authority = approvedReviewAuthority({
    batchRoot,
    resolvedConfig,
    reviewPath,
    reviewSha256,
  ...
  const selectedIds = authority.review.selectedExecutionUnitIds;
  const approvedAttention = new Set(authority.review.approvedNeedsAttention);
  const approvedUnitsById = new Map(authority.approvedInspected.repositories
    .flatMap((repository) => repository.executionUnits)
    .map((unit) => [unit.executionUnitId, unit]));
  const units = [];
  for (const executionUnitId of selectedIds) {
    ...
    const approvedUnit = approvedUnitsById.get(executionUnitId);
    const { languages: currentLanguages, ...currentStableIdentity } = unit;
    const { languages: approvedLanguages, ...approvedStableIdentity } = approvedUnit ?? {};
    if (!approvedUnit
        || !isDeepStrictEqual(currentStableIdentity, approvedStableIdentity)
        || (approvedLanguages[0] !== "unknown" && !isDeepStrictEqual(currentLanguages, approvedLanguages))) {
      throw new BatchStateError(
        `Selected execution unit differs from the approved Review: ${executionUnitId}`,
  ...
  const batchId = authority.review.batchId;
  ...
  const assessmentInput = {
    ...
    phaseApproved: true,
    inputArtifacts: {},
    decisions: authority.review.decisions,
  };
  validateAttemptInput(assessmentInput);
  const manifest = {
    batchId,
    createdAt: now,
    executionMode: "local",
    ...
    assessment: {
      ...assessmentInput,
      approvedReview: {
        reviewPath: authority.reviewPath,
        reviewSha256: authority.reviewSha256,
        reviewMarkdownPath: authority.reviewMarkdownPath,
        reviewMarkdownSha256: authority.reviewMarkdownSha256,
      },
    },
  };
  ...
  try {
    ...
    if (command === "__lease-worker") {
      ...
      result = initializeAssessmentBatch({
        ...
        reviewPath: optionValue("--review"),
        reviewSha256: optionValue("--review-sha256"),
        reviewMarkdownSha256: optionValue("--review-markdown-sha256"),
        input: jsonDocument(optionValue("--input"), "assessment initialization input"),
      });
