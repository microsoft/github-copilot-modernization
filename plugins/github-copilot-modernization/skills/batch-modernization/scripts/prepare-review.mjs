...
const batchSkillRoot = path.resolve(scriptRoot, "..");
const assessmentSkillRoot = path.resolve(batchSkillRoot, "..", "assessment");
...
const BATCH_RUNTIME_SCRIPTS = [
  "batch-assessment-report.mjs",
  "batch-attempt.mjs",
  "batch-state.mjs",
  "inspect-workspaces.mjs",
  "prepare-review.mjs",
  "probe-default-config.mjs",
  "resolve-repos.mjs",
  "schema-validator.mjs",
  "validate-result.mjs",
];
const ASSESSMENT_RUNTIME_SCRIPTS = [
  "assess-cli.mjs",
  "assess-state.mjs",
  "assess-report.mjs",
  "assess-runtime.mjs",
  "assessment-catalog.mjs",
];
...
function copyRuntimeFiles(sourceRoot, destinationRoot, names) {
  for (const name of names) {
    copyRuntimeFile(path.join(sourceRoot, name), path.join(destinationRoot, name));
  }
}

function materializeBatchRuntime(batchRoot) {
  const runtimeRoot = path.join(batchRoot, "runtime");
  const runtimeSkillsRoot = path.join(runtimeRoot, "skills");
  const batchRuntimeRoot = path.join(runtimeSkillsRoot, "batch-modernization");
  const assessmentRuntimeRoot = path.join(runtimeSkillsRoot, "assessment");

  copyRuntimeFiles(scriptRoot, path.join(batchRuntimeRoot, "scripts"), BATCH_RUNTIME_SCRIPTS);
  copyRuntimeFiles(
    path.join(batchSkillRoot, "schemas"),
    path.join(batchRuntimeRoot, "schemas"),
    fs.readdirSync(path.join(batchSkillRoot, "schemas")).filter((name) => name.endsWith(".json")),
  );
  copyRuntimeFiles(
    path.join(assessmentSkillRoot, "scripts"),
    path.join(assessmentRuntimeRoot, "scripts"),
    ASSESSMENT_RUNTIME_SCRIPTS,
  );
  copyRuntimeFile(
    path.join(assessmentSkillRoot, "scripts", "templates", "report.html"),
    path.join(assessmentRuntimeRoot, "scripts", "templates", "report.html"),
  );
  copyRuntimeFile(
    path.join(assessmentSkillRoot, "resources", "solution-mapping.json"),
    path.join(assessmentRuntimeRoot, "resources", "solution-mapping.json"),
  );
  fs.writeFileSync(path.join(runtimeRoot, ".gitignore"), "*\n!.gitignore\n", "utf8");
  return path.join(batchRuntimeRoot, "scripts", "batch-attempt.mjs");
}
...
function markdownReview(review) {
  ...
  if (review.effectiveAssessments.length > 0) {
    ...
    for (const assessment of review.effectiveAssessments) {
      const assessmentSummary = assessment.language === "unknown"
        ? "language and default domains resolve after clone"
        : `${assessment.language}; ${assessment.domains.length > 0 ? assessment.domains.join(", ") : "dependency assessment"}`;
      lines.push(`- ${assessment.executionUnitId}: ${assessmentSummary}`);
    }
    lines.push("");
  }
  ...
  if (review.status === "ready_for_approval") {
    lines.push(
      "## Approval",
      "",
      "This Review does not authorize execution. The only valid choices are exactly:",
      "",
      "- Start batch",
      "- Cancel",
      "",
...
export function formatReviewHandoff(review) {
  const fields = [
    ...
    `inspectedReposSha256: ${review.inspectedReposSha256}`,
...
export function prepareBatchReview({
  ...
  const effectiveAssessments = selected.map(({ unit }) => ({
    ...
    domains: domains ?? (unit.languages[0] === "unknown" ? [] : defaultAssessmentDomains(unit.languages[0])),
  }));
  ...
  const batchAttemptScriptPath = materializeBatchRuntime(batchRoot);
