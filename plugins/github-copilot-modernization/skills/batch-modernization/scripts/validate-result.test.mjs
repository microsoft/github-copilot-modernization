...
function writeAssessmentArtifacts(value, {
  ...
  const normalizedAssessment = path.join(
    value.workspacePath,
    ".github",
    "modernize",
    ".memory",
  ...
  const reportValue = {
    ...
    metadata: {
      ...
      name: `Report_${reportIdForRunId(selectedRunId)}`,
      ...
      targetIds: [],
      targetDisplayNames: [],
      capabilities: selectedLanguage === "java" ? ["openjdk25"] : [],
      os: selectedLanguage === "java" ? ["windows", "linux"] : [],
    ...
    projects: [{
      path: ".",
      properties: { appName: "orders", languages: [selectedLanguage], frameworks: [], tools: [] },
      incidents: [],
    }],
    rules: {},
    security: [],
  };
  const normalizedAssessmentValue = {
    schemaVersion: 1,
  ...
  fs.mkdirSync(path.dirname(normalizedAssessment), { recursive: true });
  fs.writeFileSync(normalizedAssessment, `${JSON.stringify(normalizedAssessmentValue, null, 2)}\n`);
  const payload = {
  ...
  const artifacts = { report, normalizedAssessment, html };
...
test("assessment completion binds explicit config to JSON and HTML", (t) => {
  ...
  fs.writeFileSync(created.normalizedAssessment, `${JSON.stringify({
    ...created.normalizedAssessmentValue,
    metadata: { ...created.normalizedAssessmentValue.metadata, intent: {} },
  }, null, 2)}\n`);
  assert.match(validateAttemptResult(result, policy).errors.join("\n"), /normalized assessment config/);

  fs.writeFileSync(
    created.normalizedAssessment,
    `${JSON.stringify(created.normalizedAssessmentValue, null, 2)}\n`,
  );
  const tamperedPayload = {
    ...created.payload,
    intent: {
      ...created.payload.intent,
      assessment_config: { ...assessmentConfig, targetRuntime: "java-17" },
    },
  };
  fs.writeFileSync(
    created.html,
    `<!doctype html><html><script type="application/json" id="report-data">${JSON.stringify(tamperedPayload)}</script>${"complete".repeat(2_000)}</html>`,
  );
  assert.match(validateAttemptResult(result, policy).errors.join("\n"), /HTML config/);
});

test("assessment completion rejects Single coverage provenance", (t) => {
  const value = fixture(t);
  const created = writeAssessmentArtifacts(value, { coverageSource: "default" });
  const result = baseResult("assessment", created.artifacts);
  const errors = validateAttemptResult(result, options(value, "assessment")).errors.join("\n");
  assert.match(errors, /normalized assessment coverage_source must be approved-batch/);
  assert.match(errors, /assessment HTML coverage_source must be approved-batch/);

  created.normalizedAssessmentValue.metadata.intent = {
  ...
  assert.match(
    validateAttemptResult(result, options(value, "assessment")).errors.join("\n"),
    /normalized assessment analysis_coverage does not match/,
  );
});
...
test("assessment completion rejects malformed, stale, or fabricated reports", (t) => {
  ...
  assert.match(
    validateAttemptResult(result, options(value, "assessment")).errors.join("\n"),
    /canonical report version must be 1\.0\.0/,
  );
  ...
  assert.match(
    validateAttemptResult(result, options(value, "assessment")).errors.join("\n"),
    /normalized assessment.*schemaVersion is required/,
  );

  created.normalizedAssessmentValue.schemaVersion = 2;
  created.normalizedAssessmentValue.metadata.runId = "stale-run";
  fs.writeFileSync(
    created.normalizedAssessment,
    `${JSON.stringify(created.normalizedAssessmentValue)}\n`,
  );
  const staleErrors = validateAttemptResult(result, options(value, "assessment")).errors.join("\n");
  assert.match(staleErrors, /schemaVersion/);
