...
function runCommand(executable, args, { cwd, env, spawnSyncImpl = spawnSync } = {}) {
  ...
  const repositories = resolved.repositories.map((repository) => {
    ...
    if (!fs.existsSync(repository.workspacePath)) {
      if (repository.input.url) {
        ...
        return { ...repository, preflightStatus: "needs_attention", warnings, errors };
      }
...
function cloneEnvironment(value) {
  ...
  delete env.BATCH_CLONE_URL;
  if (!value.includes("://")) return env;
  const parsed = new URL(value.trim());
  if (parsed.protocol !== "https:" || (!parsed.username && !parsed.password)) return env;

  const rawConfigCount = env.GIT_CONFIG_COUNT ?? "0";
  if (!/^\d+$/.test(rawConfigCount) || !Number.isSafeInteger(Number(rawConfigCount))) {
    throw new WorkspaceInspectionError("Git config environment is invalid", "clone_environment_invalid");
  }
  const configIndex = Number(rawConfigCount);
  ...
  env[`GIT_CONFIG_VALUE_${configIndex}`] = `Authorization: Basic ${Buffer.from(credentials, "utf8").toString("base64")}`;
  return env;
}
