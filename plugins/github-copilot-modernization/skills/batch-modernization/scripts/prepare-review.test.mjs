...
test("prepareBatchReview writes a read-only handoff", (t) => {
  ...
  assert.equal(
    review.batchAttemptScriptPath,
    path.join(review.batchRoot, "runtime", "skills", "batch-modernization", "scripts", "batch-attempt.mjs"),
  );
  assert.equal(fs.statSync(review.batchAttemptScriptPath).isFile(), true);
  assert.equal(
    fs.statSync(path.join(review.batchRoot, "runtime", "skills", "assessment", "scripts", "assess-cli.mjs")).isFile(),
    true,
  );
  assert.match(review.reviewSha256, /^[a-f0-9]{64}$/);
  assert.match(review.reviewMarkdownSha256, /^[a-f0-9]{64}$/);
  assert.match(review.inspectedReposSha256, /^[a-f0-9]{64}$/);
  ...
  assert.match(handoff, /The only valid choices are exactly:/);
  assert.match(handoff, /^- Start batch$/m);
  assert.match(handoff, /^- Cancel$/m);
  ...
  assert.match(handoff, new RegExp(`inspectedReposSha256: ${review.inspectedReposSha256}`));
...
test("prepareBatchReview keeps clone-required repositories selectable", (t) => {
  const launchRoot = fs.mkdtempSync(path.join(os.tmpdir(), "batch-review-remote-"));
  t.after(() => fs.rmSync(launchRoot, { recursive: true, force: true }));
  const configPath = path.join(launchRoot, "repos.json");
  fs.writeFileSync(configPath, `${JSON.stringify({
    repos: [{ name: "remote-only", url: "https://github.com/contoso/remote-only.git" }],
  }, null, 2)}\n`);
  ...
  const review = prepareBatchReview({
    ...
    launchRoot,
    allowedRoots: [launchRoot],
    batchId: "remote-review",
  });

  assert.equal(review.status, "ready_for_approval");
  assert.deepEqual(review.selectedExecutionUnitIds, ["remote-only"]);
  assert.deepEqual(review.approvedNeedsAttention, ["remote-only"]);
  assert.deepEqual(review.groups.cloneRequired[0].executionUnitIds, ["remote-only"]);
  assert.deepEqual(review.effectiveAssessments, [{
    executionUnitId: "remote-only",
    language: "unknown",
    domains: [],
  }]);
  assert.match(review.markdown, /language and default domains resolve after clone/);
});
