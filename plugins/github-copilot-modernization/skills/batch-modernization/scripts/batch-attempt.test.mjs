...
import { cloneRepository, inspectResolvedRepositories } from "./inspect-workspaces.mjs";
import { prepareBatchReview } from "./prepare-review.mjs";
...
function createJavaGitProject(workspacePath, remoteUrl) {
  fs.mkdirSync(workspacePath, { recursive: true });
  git(workspacePath, "init");
  git(workspacePath, "config", "user.email", "batch@example.test");
  git(workspacePath, "config", "user.name", "Batch Test");
  fs.writeFileSync(path.join(workspacePath, "pom.xml"), "<project/>\n");
  git(workspacePath, "add", "pom.xml");
  git(workspacePath, "commit", "-m", "initial");
  git(workspacePath, "branch", "-M", "main");
  git(workspacePath, "remote", "add", "origin", remoteUrl);
}
...
function approvedReviewFiles({
  batchRoot,
  resolvedConfig: inspectedConfig,
  batchId,
  selection,
  decisions,
}) {
  ...
  const markdown = "# Approved Batch Review";
  fs.writeFileSync(inspectedReposPath, `${JSON.stringify(inspectedConfig, null, 2)}\n`);
  fs.writeFileSync(reviewMarkdownPath, `${markdown}\n`);
  fs.writeFileSync(reviewPath, `${JSON.stringify({
    ...
    reviewMarkdownPath,
    inspectedReposPath,
    batchAttemptScriptPath: batchAttemptCli,
    configSha256: inspectedConfig.configSha256,
    inspectedReposSha256: fileSha256(inspectedReposPath),
    selectedExecutionUnitIds: selection.executionUnitIds,
    approvedNeedsAttention: selection.approvedNeedsAttention,
    decisions,
    markdown,
  }, null, 2)}\n`);
  return {
...
function initializeApprovedAssessmentBatch({
  batchRoot,
  resolvedConfig: inspectedConfig,
  selection,
  input,
  ...
  const reviewFiles = approvedReviewFiles({
    batchRoot,
    resolvedConfig: inspectedConfig,
  ...
  return initializeAssessmentBatch({
    batchRoot,
    resolvedConfig: inspectedConfig,
    ...reviewFiles,
    input: { userRequest: input.userRequest },
    now,
  });
}
...
function createAssessmentArtifacts(started, name, {
  ...
  const report = {
    ...
    metadata: {
      ...
      name: `Report_${reportIdForRunId(request.runId)}`,
      analysisStartTime: "2026-08-17T12:00:00Z",
      analysisEndTime: "2026-08-17T12:01:00Z",
      status: "completed",
      domains: request.decisions.domains,
      mode: request.decisions.analysisCoverage,
      targetIds: [],
      targetDisplayNames: [],
      capabilities: request.language === "java" ? ["openjdk25"] : [],
      os: request.language === "java" ? ["windows", "linux"] : [],
      minimumCveSeverity: assessmentConfig.minimumCveSeverity ?? "high",
      cveScanScope: assessmentConfig.cveScanScope ?? "direct",
    },
    summary: { totalProjects: 1, totalIssues: 0, totalIncidents: 0, totalEffort: 0 },
    projects: [{
      path: ".",
      properties: { appName: name, languages: [request.language], frameworks: [], tools: [] },
      incidents: [],
    }],
    rules: {},
    security: [],
  ...
  const intent = {
    analysis_coverage: request.decisions.analysisCoverage,
    coverage_source: "approved-batch",
    ...(Object.keys(assessmentConfig).length > 0 ? { assessment_config: assessmentConfig } : {}),
  };
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify(normalizedAssessment, null, 2)}\n`,
  );
  ...
  const artifacts = {
    report: reportPath,
    normalizedAssessment: normalizedAssessmentPath,
    html: htmlPath,
  };
...
test("assessment attempt is bound to one unit and commits verified artifacts", (t) => {
  ...
  const publishedReportPath = path.join(userReportRoot, "repos", "orders.api", "report.json");
  assert.equal(fs.existsSync(publishedReportPath), true);
  assert.deepEqual(fs.readFileSync(publishedReportPath), fs.readFileSync(reportPath));
  assert.equal(
    fs.existsSync(path.join(userReportRoot, "repos", "orders.api", "normalized-assessment.json")),
    false,
  );
...
test("lease session retains ownership across stateless CLI processes", (t) => {
  ...
  initializeApprovedAssessmentBatch({
    batchRoot,
...
test("initialization derives pending units only from approved preflight selections", (t) => {
  ...
  const initialized = initializeApprovedAssessmentBatch({
...
test("initialization rejects authority substituted outside the approved Review", (t) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "batch-review-authority-"));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  const alphaPath = path.join(root, "alpha");
  const betaPath = path.join(root, "beta");
  fs.mkdirSync(alphaPath);
  fs.mkdirSync(betaPath);
  const reviewedConfig = resolvedConfig(root, [
    {
      repoId: "alpha",
      workspacePath: alphaPath,
      executionUnits: [assessmentUnit({ repoId: "alpha", workspacePath: alphaPath, language: "java" })],
    },
    {
      repoId: "beta",
      workspacePath: betaPath,
      executionUnits: [assessmentUnit({ repoId: "beta", workspacePath: betaPath, language: "java" })],
    },
  ]);
  const decisions = { domains: ["cloud-readiness"], analysisCoverage: "issue-only", maxConcurrency: 1 };
  const selection = { executionUnitIds: ["alpha"], approvedNeedsAttention: [] };
  const batchRoot = path.join(root, "approved-batch");
  const reviewFiles = approvedReviewFiles({
    batchRoot,
    resolvedConfig: reviewedConfig,
  ...
  const initialization = {
    ...
    resolvedConfig: reviewedConfig,
    ...reviewFiles,
    input: { userRequest: "Assess approved repositories" },
  };

  assert.throws(
    () => initializeAssessmentBatch({
      ...initialization,
      selection: { executionUnitIds: ["beta"], approvedNeedsAttention: [] },
    }),
    (error) => error.code === "invalid_initialization_authority",
  );
  assert.throws(
    () => initializeAssessmentBatch({
      ...initialization,
      input: { ...initialization.input, decisions: { ...decisions, analysisCoverage: "full" } },
    }),
    (error) => error.code === "invalid_attempt_input" && /decisions/.test(error.message),
  );
  assert.throws(
    () => initializeAssessmentBatch({ ...initialization, reviewSha256: "0".repeat(64) }),
    (error) => error.code === "review_digest_mismatch",
  );

  const substitutedConfig = structuredClone(reviewedConfig);
  substitutedConfig.repositories[0].workspacePath = betaPath;
  substitutedConfig.repositories[0].input.path = betaPath;
  substitutedConfig.repositories[0].executionUnits[0] = assessmentUnit({
    repoId: "alpha",
    workspacePath: betaPath,
    language: "java",
  });
  assert.throws(
    () => initializeAssessmentBatch({ ...initialization, resolvedConfig: substitutedConfig }),
    (error) => error.code === "review_selection_mismatch",
  );
  assert.equal(fs.existsSync(path.join(batchRoot, "manifest.json")), false);
});

test("URL-only Review flows through approved clone, reinspection, and initialization", async (t) => {
  const launchRoot = fs.mkdtempSync(path.join(os.tmpdir(), "batch-remote-flow-"));
  t.after(() => fs.rmSync(launchRoot, { recursive: true, force: true }));
  const remoteUrl = "https://github.com/contoso/remote-orders.git";
  const sourcePath = path.join(launchRoot, "clone-source");
  createJavaGitProject(sourcePath, remoteUrl);
  const configPath = path.join(launchRoot, "repos.json");
  fs.writeFileSync(configPath, `${JSON.stringify({
    repos: [{ name: "remote-orders", url: remoteUrl, branch: "main" }],
  }, null, 2)}\n`);
  ...
  assert.deepEqual(review.selectedExecutionUnitIds, ["remote-orders"]);
  assert.deepEqual(review.effectiveAssessments[0].languages, undefined);
  assert.equal(review.effectiveAssessments[0].language, "unknown");

  const resolved = JSON.parse(fs.readFileSync(review.resolvedReposPath, "utf8"));
  const targetPath = resolved.repositories[0].workspacePath;
  cloneRepository({
    url: remoteUrl,
    targetPath,
    allowedRoot: launchRoot,
    branch: "main",
    spawnSyncImpl: (_command, args) => {
      fs.cpSync(sourcePath, args.at(-1), { recursive: true });
      return { status: 0, stdout: "", stderr: "" };
    },
  });
  const inspected = inspectResolvedRepositories(resolved, { allowedRoots: [launchRoot] });
  assert.equal(inspected.repositories[0].preflightStatus, "ready");
  assert.deepEqual(inspected.repositories[0].executionUnits[0].languages, ["java"]);

  const runtimeBatchAttempt = await import(pathToFileURL(review.batchAttemptScriptPath).href);
  const initialized = runtimeBatchAttempt.initializeAssessmentBatch({
    ...
    input: { userRequest: "Assess my app" },
  });
  assert.deepEqual(initialized.manifest.selectedExecutionUnitIds, ["remote-orders"]);
  assert.deepEqual(initialized.manifest.assessment.decisions, {
...
test("initialization rejects include-path units before creating batch state", (t) => {
  ...
  const inspectedConfig = resolvedConfig(root, [{
    repoId: "portfolio",
    workspacePath: gitRoot,
    executionUnits: units,
    includePaths: ["services/api", "services/web"],
  }]);
  fs.writeFileSync(resolvedPath, `${JSON.stringify(inspectedConfig, null, 2)}\n`);
  const reviewFiles = approvedReviewFiles({
    batchRoot: cliRoot,
    resolvedConfig: inspectedConfig,
  ...
  const cliResult = runBatchAttemptCli([
    ...
    "--review", reviewFiles.reviewPath,
    "--review-sha256", reviewFiles.reviewSha256,
    "--review-markdown-sha256", reviewFiles.reviewMarkdownSha256,
    "--input", inputPath,
  ]);
