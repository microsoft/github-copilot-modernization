  ...
  for (const result of results) {
    ...
    if (USABLE_STATUSES.has(result.status)) {
      if (!result.artifacts.report
          || !result.artifacts.normalizedAssessment
          || !result.artifacts.html) {
        throw new BatchStateError(
          `Usable assessment result has no canonical, normalized, and HTML report set: ${identity}`,
      ...
      const normalizedAssessment = JSON.parse(fs.readFileSync(
        result.artifacts.normalizedAssessment,
        "utf8",
      ));
      repositoryAssessmentConfig = assessmentConfig(
        normalizedAssessment.metadata.intent?.assessment_config,
      );
      const singleSummary = singleReportSummary(htmlPath, result.language);
      metrics = { ...reportMetrics(normalizedAssessment), ...singleSummary };
      ...
      for (const domain of normalizedAssessment.metadata.domains) effectiveDomains.add(domain);
      if (normalizedAssessment.metadata?.analysisStartTime) {
        analysisStarts.push(asDate(
          normalizedAssessment.metadata.analysisStartTime,
          "Normalized assessment analysisStartTime",
        ));
      }
      if (normalizedAssessment.metadata?.analysisEndTime) {
        analysisEnds.push(asDate(
          normalizedAssessment.metadata.analysisEndTime,
          "Normalized assessment analysisEndTime",
        ));
      }
      const facts = [];
      const digests = {
