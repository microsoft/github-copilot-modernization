...
test("execution coordinator can read required coordination metadata", () => {
  const agent = fs.readFileSync(executionCoordinatorAgentPath, "utf8");
  const tools = agent.match(/^tools:\r?\n((?:\s+- [^\r\n]+\r?\n?)*)/m)?.[1] ?? "";

  assert.match(tools, /^\s+- read$/m);
  assert.match(agent, /FIRST WORK ACTION MUST BE constraint\/plan loading/);
});
...
test("exposes execution workers transitively but rejects direct router delegation", () => {
  const modernizeAgent = fs.readFileSync(modernizeAgentPath, "utf8");
  for (const worker of [
    ...
    assert.match(modernizeAgent, new RegExp(`^  - ${worker}$`, "m"));
...
test("modernize wires the scoped delegation guards", () => {
  ...
  assert.match(agent, /PostToolUse:/);
  assert.match(agent, /guardModernizeDelegation\.mjs.*--post/);
