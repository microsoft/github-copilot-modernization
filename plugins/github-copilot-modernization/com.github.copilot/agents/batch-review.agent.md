---
name: batch-review
description: Use immediately after Batch mode is selected to prepare the required read-only BATCH_REVIEW_READY approval handoff; this is the only agent that prepares a Batch Review
...
hooks:
  PreToolUse:
    - type: command
      command: node "$APPMOD_HOOK_SCRIPTS_DIR/guardModernizeDelegation.mjs" --review
      windows: "powershell -ExecutionPolicy Bypass -NonInteractive -Command \"& node (Join-Path $env:APPMOD_HOOK_SCRIPTS_DIR 'guardModernizeDelegation.mjs') --review\""
  SubagentStart:
    - type: command
...
inspectedReposSha256: <64 lowercase hex characters>
