...
agents:
  ...
  # VS Code validates nested delegation against the root agent's allowlist.
  ...
  # BEGIN PLATFORM BATCH AGENTS (plugin)
  - batch-mode-probe
  - batch-review
  - batch-coordinator
  # END PLATFORM BATCH AGENTS
hooks:
  PreToolUse:
    - type: command
  ...
  # BEGIN PLATFORM BATCH HOOKS (plugin)
  PostToolUse:
    - type: command
      command: node "$APPMOD_HOOK_SCRIPTS_DIR/guardModernizeDelegation.mjs" --post
      windows: "powershell -ExecutionPolicy Bypass -NonInteractive -Command \"& node (Join-Path $env:APPMOD_HOOK_SCRIPTS_DIR 'guardModernizeDelegation.mjs') --post\""
  # END PLATFORM BATCH HOOKS
  SessionStart:
    - type: command
